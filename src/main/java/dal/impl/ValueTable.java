    ...
    static ValueTable of(
        ...
        */
        ...
        for (Method method : methods) {
            ...
            // Skip compiler-generated bridge/synthetic methods. A record that implements a
            // generic interface (e.g. sprouts.HasId<String> via an `id` component) gets an
            // erased bridge accessor (`Object id()`) alongside the real one (`String id()`).
            // Both share the component name, so without this guard the bridge would either be
            // rejected (its erased return type is not a primitive/Value/tuple) or register a
            // duplicate field. The canonical record accessor is never bridge nor synthetic.
